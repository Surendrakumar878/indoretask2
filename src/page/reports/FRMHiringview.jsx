...
const FRMHiringview = () => {
  ...
  useEffect(() => {
    setDate(
      userData.filter((res) =>
        res.vehicle_no.toLowerCase().includes(vehicle.toLowerCase())
      )
    );
  }, [vehicle]);
  useEffect(() => {
  ...
  useEffect(() => {
    setDate(
      userData.filter((res) =>
        res.hire_owner_name
          .toLowerCase()
          .includes(hire_owner_name.toLowerCase())
      )
    );
  }, [hire_owner_name]);
  useEffect(() => {
      ...
      <div className={schedule.scheduleview}>
        <Row>
          <Col className={schedule.select_option_col}>
            <label style={{ paddingLeft: "0px" }}>HIRE TIME/DATE </label>
            <br />
            <input
          ...
          <Col className={schedule.select_option_col}>
            <label style={{ paddingLeft: "0px" }}>VEHICLE NO :</label>
            <br />
            <Input
          ...
          <Col className={schedule.select_option_col}>
            <label style={{ paddingLeft: "0px" }}>OWNER NAME </label>
            <br />
            <Input
              placeholder="Search by OWNER NAME"
              value={hire_owner_name}
              onChange={(e) => sethire_owner_name(e.target.value)}
              style={{ width: "90%" }}
            />
          </Col>
          <Col className={schedule.select_option_col}>
            <label style={{ paddingLeft: "" }}>TRIP STATUS </label>
            <br />
        ...
        <table className="w-full" style={{ marginTop: "20px" }}>
          ...
          {data.slice().map((items, index) => (
            <tr key={index}>
              ...
              <td className="text-center">{items.hire_id}</td>
              <td className="text-center">{items.hire_date_time}</td>
              <td className="text-center">{items.vehicle_no}</td>
              <td className="text-center">{items.hire_owner_name}</td>
              <td className="text-center">{items.trip_status}</td>
           
              <td  className="text-center" id={schedule.transaction_boder}>
                {" "}
                <Link> view</Link>
              </td>
            </tr>
